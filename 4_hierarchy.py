...
from base_func import cosim, nn_dist
...
stat_dict = np.load('results/stats_deezer.npy', allow_pickle=True).item() # stats_deezer, stats_genre_apm, stats_mood_apm
training_names = np.load('weights/learned_deezer_cav.npy', allow_pickle=True)[2]
...
def regen_subgraph(idx, S):
    ''' Regen graph on only the available subet of nodes '''
    S_red = S[idx][:,idx]
    return gen_sim_graph(S_red)
...
def regen_subhierarchy(idx, S, names, centrality_vec):
    ...
    similarity, adjacency = regen_subgraph(idx, S)
...
llm_intersect, llm_similarity, sim_llm_graph = gen_opt_graph(names_llm, emb_llm, threshold = 0.43)
...
usg_intersect, usg_similarity, sim_usg_graph = gen_opt_graph(names_usageemb, emb_usg, use_cosim = False, threshold = -0.8, gen_graph = True)
...
sdoh_intersect, sdoh_similarity, sim_sdoh_graph = gen_opt_graph(names_w2v_sdoh, emb_w2v_sdoh, threshold = 0.68)
...
mwon_intersect, mwon_similarity, sim_mwon_graph = gen_opt_graph(names_w2v_mwon, emb_w2v_mwon, threshold = 0.38)
mwon_centrality_vec = nx.centrality.betweenness_centrality(sim_mwon_graph)

# Audio
weights_np, biases_np, names_cav = np.load('weights/learned_deezer_cav.npy', allow_pickle = True)
