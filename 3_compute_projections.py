...
from model_cav import CAV_Predictor
...
predictor = CAV_Predictor(
        bottleneck, CAV_WEIGHT_PATH, temporal_pooling = True,
        activation = False )
...
stats = defaultdict( lambda: { 'ex': np.zeros(len(predictor.labels)),
...
for tag in predictor.labels:
    ...
    test_data_it = data_loader.create_reader_from_set_tf_iterator(songs_split['test']['pos'])
    ...
    for X in test_data_it:
        ...
        for pool_layer in range(4):
            ...
            stats[subtag]['ex2'] = stats[subtag]['ex2'] + np.mean(np.square(V[pool_layer]), 0)
            stats[subtag]['esigx'] = stats[subtag]['esigx'] + np.mean(sigV[pool_layer], 0)
            stats[subtag]['esigx2'] = stats[subtag]['esigx2'] + np.mean(np.square(sigV[pool_layer]), 0)
            stats[subtag]['n'] += 1
