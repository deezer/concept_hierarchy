...
class ImbalancedDataLoader:
    ...
    def pad_small(x, bonus_pad = 20):
        ...
        return tf.pad(x, ((bonus_pad, lacking_pad + bonus_pad), (0, 0)),
    ...
    def slice_along_audio(x):
        ''' Takes a random slice of the spectrogram for data augmentation '''
        max_split = tf.shape(x)[0] // INPUT_SIZE[0]
        cut_index = tf.range(0, tf.shape(x)[0] - INPUT_SIZE[0], INPUT_SIZE[0] // 2)
        splited = tf.map_fn(lambda i: x[i:i+INPUT_SIZE[0]],
                        cut_index,
                        fn_output_signature = tf.float32 )
        return tf.stack( splited )
    ...
    def create_tf_iterator(self, sample_lists, mode = 'train'):
        '''
        ...
        def open_batch(ds):
            # reutilises the same song several times -> kinda a cache of recent items
            ds = ds.flat_map(lambda x:
                    tf.data.Dataset.from_tensors(x).repeat(self.params['repeat'])
                )

            ds = ds.map(self.random_audio_slice)
            ds = ds.shuffle(self.params['repeat'] * self.params['batch_size'] * 5)
            ds = ds.batch(self.params['batch_size'] // 2)
            return ds

        pos_ds = open_batch(pos_ds)
        neg_ds = open_batch(neg_ds)
    ...
    def create_reader_from_set_tf_iterator(self, song_set):
        ''' given predefined splitted song set, do stuff'''
