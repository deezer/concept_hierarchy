...
from tensorflow.keras.layers import Input, Dense, Flatten, Lambda, Dropout, BatchNormalization
...
class CAV_Regressor:
    '''
        This is solely used to train a sigle regressor and learn a CAV.
    '''
    ...
    def build(self, embedder, temporal_pooling):
        ...
        for i, emb in enumerate(f):
            ...
            if temporal_pooling and len(emb.shape) == 4:
                t = Lambda(lambda x: tf.reduce_mean(x, axis = 1),
                            name='temporal_pooling_'+str(i) )(t)
            y, layer = self.build_single_pipe(t, name = str(i) )
            lms.append(layer)
            output.append(y)
        self.lm_layers = lms
        self.model = Model(inputs = X, outputs = output)
        self.model.compile( loss = ['binary_crossentropy'] * len(output),
                        optimizer=Adam(1e-3),
                        metrics=['acc'] )
...
class CAV_Predictor:
    ...
    '''
    def __init__(self, embedder, wpath,
